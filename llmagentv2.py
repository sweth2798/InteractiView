...
def generateSqlQuery(input):
    ...
    ERROR', 'RESUBMITTING'. 'POLLABLE' also comes under failed case.'TransactionAttribute' table has the following columns 'TransactionID','AttributeID', 
...
def generateResponse(input, sqlQuery, resultSet):
    prompt = """
                ...
                {"type": "table", "columns": ["column1", "column2", ...], "data": [[A, B, ...], [D, C, ...], ...]}
                
                If the query requires creating a bar chart, reply as follows:
                {"type": "bar", "x-axis": ["A", "B", "C", ...], "y-axis": [25, 24, 10, ...]}

                If the query requires creating a line chart, reply as follows:
                {"type": "line", "x-axis": ["A", "B", "C", ...], "y-axis": [25, 24, 10, ...]}

                If the query requires creating a pie chart, reply as follows:
                {"type": "pie", "x-axis": ["A", "B", "C", ...], "y-axis": [25, 24, 10, ...]}

                If the query requires creating a wordcloud, reply as follows:
                {"type": "wordcloud", "x-axis": ["A", "B", "C", ...], "y-axis": [25, 24, 10, ...]

                There can only be 4 types of chart, "bar", "line", "pie" and "wordcloud".

                If it is just asking a question that requires neither, reply as follows:
                {"type": "string", "values": "Answer to the query"}
                Example:
                {"type": "string", "values": "The title with the highest rating is 'Gilead'"}

                If you do not know the answer, reply as follows:
                {"type": "string", "values": "I do not know."}
